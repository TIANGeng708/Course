...
//            上下车测试数据全为0
    ...
    public void Vehicleperation_task06_example5() {
        try {
            ...
            //检测更新传入2.5f给getFare()方法是否正确
            assertEquals(25,(int)(V1.getIncome()*10),"\"your getFareMethod is wrong!\"");
           //比较更新后income数据是否正确
            assertFalse(V1.getFare(-5),"your getFareMethod is wrong!");
           //失败的样例，getFare()不可以传入负值
            assertEquals(2.5f,V1.getIncome(),"your getFareMethod is wrong!");
          //查看传入负值后income是否未改变
        }catch (Exception e){
            e.printStackTrace();
    ...
    public void Vehicleperation_task07_example06() {
        try {
            ...
            assertFalse( V1.updateCntOnStation("staionF", 14, 8));//TODO
            //错误的数据不会被添加
            Class clazz = VehicleRun.class;
            Field nameA = clazz.getDeclaredField("pCntOff");
            Field nameB = clazz.getDeclaredField("pCntOn");
            ...
            assertEquals(On[r3],pCntOnArray[r3]);
            assertEquals(On[r4],pCntOnArray[r4]);
            assertEquals(Off[r4],pCntOffArray[r4]);
            assertEquals(Off[r4],pCntOffArray[r4]);
        }catch (Exception e){
            fail("updateCntOnStationRunError");
        }
