...
public class BusTestDemo {
    ...
    void arriveStationTest1(){
        ...
        for (int i = 0; i < pCntOn.length; i++) {
            assertEquals(0, pCntOn[i]);
            assertEquals(0, pCntOff[i]);
        } //check initialization

        b.arriveStation(cvi, xs, pgo, pgf); //A arrived

        //check
        assertEquals(3, pCntOn[0]);
        assertEquals(0, pCntOff[0]); //check if numbers of passengers get on and get off are right

        assertEquals("A", p1.getBoardingStation());
        assertEquals("A", p2.getBoardingStation());
        assertEquals("A", p3.getBoardingStation()); //check if boarding station be changed correctly

        assertEquals(90.0f, p1.getBalance(), 0);
        assertEquals(90.0f, p2.getBalance(), 0);
        assertEquals(90.0f, p3.getBalance(), 0); //check if balance be changed correctly
        ...
        assertEquals(3, onbus); //check if amount of people in bus is right
        ...
        //check, same as above
        assertEquals(2, pCntOn[1]);
        assertEquals(3, pCntOff[1]);

        assertNull(p1.getBoardingStation());
        assertNull(p2.getBoardingStation());
        assertNull(p3.getBoardingStation());
        assertEquals("B", p4.getBoardingStation());
        assertEquals("B", p5.getBoardingStation());

        assertEquals(90.0f, p1.getBalance(), 0);
        assertEquals(90.0f, p2.getBalance(), 0);
        assertEquals(90.0f, p3.getBalance(), 0);
        assertEquals(90.0f, p4.getBalance(), 0);
        assertEquals(90.0f, p5.getBalance(), 0);
        ...
        //check, same as above
        assertEquals(0, pCntOn[2]);
        assertEquals(2, pCntOff[2]);

        assertNull(p4.getBoardingStation());
        assertNull(p5.getBoardingStation());
    ...
    void arriveStationTest2(){
        ...
        assertFalse(b.arriveStation(cvi, xs, pgo, pgf)); //check if return false
    }
    ...
    void arriveStationTest3(){
        ...
        assertFalse(b.arriveStation(cvi, xs, pgo, pgf)); //C arrived(the last station), check if it return false
    }
